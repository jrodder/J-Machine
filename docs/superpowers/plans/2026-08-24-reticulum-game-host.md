...
7. **Per-turn delta replies (2026-08-25, supersedes 1)** — discovered in live phone testing: Sideband renders each LXMF message as a chat bubble, so the cumulative reply re-posts the entire game every turn (O(n²) text in the scrollback). Now: first-contact reply = the load/restore batch (+ that line's turn if attached) — all-new data; every subsequent reply = ONLY the new turn's text (an empty turn may be an empty message). The chat scrollback is the transcript; the phone accumulates. Empty message = deliberate full-transcript state re-fetch (recovery path). A host restart is the one transcript reset (restored batch empty for a boundary save — see 1), safe because the phone's scrollback already has the pre-boundary text. Consequences: `jclient play` prints replies verbatim (no increment math) and waits by arrival index, not content — LXMF/RNS dedupe retransmissions at the delivery layer (spec §2), but identical deltas (two "Taken." turns) would be swallowed by content-based seen-sets. `st.transcript` is kept in `GameState` for the empty-message re-fetch.
